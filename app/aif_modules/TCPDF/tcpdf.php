...
class TCPDF {
	...
	protected function getHtmlDomArray($html) {
		...
		$matches = array();
if (preg_match_all('/<cssarray>([^\<]*?)<\/cssarray>/is', $html ? $html:'', $matches) > 0) {		
		...
		$matches = array();
		if (preg_match_all('/<link([^\>]*?)>/is', $html ? $html:'', $matches) > 0) {
		...
		// extract style tags
		$matches = array();
		if (preg_match_all('/<style([^\>]*?)>([^\<]*?)<\/style>/is', $html ? $html:'', $matches) > 0) {
