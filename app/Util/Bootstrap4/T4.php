...
class UtilBootstrap4T4 extends UtilBootstrap4 {
  static public function pages($current, $pages, $url, $count = 6)
  {

    $ht = '<div class="pull-right"><ul class="pagination pagination-sm not-radius justify-content-end m-0 p-0">';
    $n = 1;
  ...
  public static function options($rs = array(), $text = 'Acciones', $color = 'primary')
  { $ico = '';
    $ht = '<div class="dropdown">' .
      '<a href="#" class="text-'.$color.' dropdown-toggle no-caret" data-toggle="dropdown">' .
      //'<i class="fa fa-' . $ico . '"></i>' .
      ($text ? '<span> ' . $text . ' </span>' : '') .
      '<span class="caret text-title">' . '</span>' .
      '</a>' .
  ...
  public static function optionsActions($rs = array(), $text = '', $color = 'white') {
    return self::options($rs, $text, $color);
  }
